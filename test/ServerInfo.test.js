...
describe('ServerInfo', function() {
  ...
  describe('.show()', function() {
    ...
    it('prints QR code with terminal renderer to console', async function() {
      ...
      const qrCodeSize = 27;
      const regexp = new RegExp(`((${ansiiEscapeBlackSquare}|${ansiiEscapeWhiteSquare})+\\n){${qrCodeSize}}$`, 'gm');
  ...
  describe('.selectUrlForQRCode()', function() {
    ...
    it('returns address of only interface', async function() {
      const serverInfo = new ServerInfo({port: '8080', wsPort: '8081'}, [new URL('http://127.0.0.1:8080')]);
      stub(serverInfo, 'getFirstWifiAddress');

      const url = await serverInfo.selectUrlForQRCode();
